...
class _ChatMainScreenState extends State<ChatMainScreen> {
  ...
  void initState() {
    ...
    WidgetsBinding.instance.addPostFrameCallback((_) async {
      await _loadChatClickAndFetchQuestions();
      await _loadCommunityRandomQuestion();
    });
  }
  ...
  Set<String> _normSet(Iterable<String> xs) => xs.map(_norm).toSet();
  ...
  Future<void> _loadChatClickAndFetchQuestions() async {
    ...
    if (snapshot.exists) {
      ...
      chatClick = 1;
      await docRef.set({'book_id': widget.title, 'chat_click': chatClick},
          SetOptions(merge: true));
    }

    final level = chatClick < 3
        ? 1
        : (chatClick < 6
            ? 2
            : 3);
  ...
  Future<void> _loadCommunityRandomQuestion() async {
    ...
    try {
      final normHints = _normSet(hintQuestions);
      ...
      for (final doc in qs.docs) {
        ...
        if (normHints.contains(_norm(text))) continue;
  ...
  Future<void> _incrementChatClickAndNavigate(String message) async {
    ...
    if (snap.exists) {
      ...
      await docRef.set({'book_id': widget.title, 'chat_click': 1});
    }
              ...
              child: SingleChildScrollView(
                ...
                child: Column(
                  children: [
                    ...
                    SvgPicture.asset('assets/images/main_Chaekmeong_1.svg',
                    ...
                    const Text('아래는 많은 독서가들이 궁금해하는 것들이에요!',
                        style:
                            TextStyle(fontSize: 14, color: Color(0xff777777))),
                    const SizedBox(height: 16),
                    buildHintChips(),
                    const SizedBox(height: 12),
                    const Text('다른 독자들이 한 질문은 뭘까요?',
                        style:
  ...
  Widget buildCommunityChip() {
    if (communityHint == null) return const SizedBox.shrink();
    final isDup = _normSet(hintQuestions).contains(_norm(communityHint!));
    if (isDup) return const SizedBox.shrink();
        ...
        GestureDetector(
          onTap: () {
            if (communityHint != null) {
              _incrementChatClickAndNavigate(communityHint!);
            }
          },
          child: Chip(
